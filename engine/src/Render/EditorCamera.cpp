    ...
    if (!m_bIsCameraActionsBinded)
    {
        ...
        m_inputEvents.push_back({Input::AddEventListener(InputEventType::KeyReleased,
                                                         [this](const InputEventPointer &event)
                                                         {
                                                             ...
                                                             if (Input::IsMouseButtonHeld(MouseButton::Right))
                                                             {
                                                                 SetCameraProperties(key, InputEventType::KeyReleased);
                                                             }
                                                         }),
        ...
        m_inputEvents.push_back({Input::AddEventListener(InputEventType::MouseDelta,
                                                         [this](const InputEventPointer &event)
                                                         {
                                                             const MouseMovedEvent *mouseEvent =
        ...
        m_inputEvents.push_back({Input::AddEventListener(InputEventType::MouseScrolled,
                                                         [this](const InputEventPointer &event)
                                                         {
                                                             const MouseScrolledEvent *mouseEvent =
                                                                 static_cast<const MouseScrolledEvent *>(event.get());
                                                             if (Input::IsMouseButtonHeld(MouseButton::Right))
                                                             {
                                                                 if (mouseEvent->GetYOffset() > 0) IncreaseSpeed();
                                                                 else if (mouseEvent->GetYOffset() < 0) DecreaseSpeed();
                                                             }
                                                         }),
                                 InputEventType::MouseScrolled});
...
{
    float currCamSpeed = 0.1f * m_deltaTime * m_cameraSpeed * m_editorSpeed;
    if (m_bUseAcceleration) currCamSpeed *= m_cameraAcceleration;

    switch (key)
    {
    case Blainn::KeyCode::A:
        MoveRight(-currCamSpeed);
        break;
    case Blainn::KeyCode::D:
        MoveRight(currCamSpeed);
        break;
    case Blainn::KeyCode::S:
        MoveForward(-currCamSpeed);
        break;
    case Blainn::KeyCode::W:
        MoveForward(currCamSpeed);
        break;
    case Blainn::KeyCode::Q:
        MoveUp(-currCamSpeed);
        break;
    case Blainn::KeyCode::E:
        MoveUp(currCamSpeed);
        break;
    }
...
{
    m_editorSpeed = eastl::clamp(m_editorSpeed + m_speedStep, 0.0f, m_maxEditorSpeed);
}
...
void EditorCamera::SetCameraProperties(const KeyCode key, InputEventType eventType)
{
    switch (key)
    {
    case Blainn::KeyCode::Shift:
        if (eventType == InputEventType::KeyPressed) SetAcceleration(true);
        else if (eventType == InputEventType::KeyReleased) SetAcceleration(false);
        break;
    }
}
