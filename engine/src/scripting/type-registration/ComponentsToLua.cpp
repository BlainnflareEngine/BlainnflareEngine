...
{
    // Basic components
    sol::usertype<IDComponent> IDComponentType = luaState.new_usertype<IDComponent>(
        "IDComponent", sol::constructors<IDComponent()>());
    ...
    sol::usertype<TagComponent> TagComponentType = luaState.new_usertype<TagComponent>(
        "TagComponent", sol::constructors<TagComponent(),
                                          TagComponent(const eastl::string &)>());
    TagComponentType["Tag"] = sol::property(
        [](const TagComponent &t) { return std::string(t.Tag.c_str()); },
    ...
    CameraComponentType.set_function("SetNearPlane",
        [](CameraComponent &camera, float nearPlane) { camera.camera.SetNearZ(nearPlane); });
