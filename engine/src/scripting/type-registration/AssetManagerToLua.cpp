...
void Blainn::RegisterAssetManagerTypes(sol::state &luaState)
{
    ...
    sol::usertype<TextureHandle> TextureHandleType = luaState.new_usertype<TextureHandle>(
        "TextureHandle", sol::no_constructor,
    ...
    sol::usertype<MaterialHandle> MaterialHandleType = luaState.new_usertype<MaterialHandle>(
        "MaterialHandle", sol::no_constructor,
        sol::base_classes, sol::bases<Handle>());
    MaterialHandleType.set_function("GetMaterial", &MaterialHandle::GetMaterial);

    sol::usertype<MeshHandle> MeshHandleType = luaState.new_usertype<MeshHandle>(
        "MeshHandle", sol::no_constructor,
        sol::base_classes, sol::bases<Handle>());
    ...
    manager.set_function("OpenScene",
        [](const std::string &path) { AssetManager::OpenScene(Path(path)); });
    manager.set_function("CreateScene",
        [](const std::string &path) { AssetManager::CreateScene(Path(path)); });
